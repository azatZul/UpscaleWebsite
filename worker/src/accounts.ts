...
export async function deviceAllowance(db: D1Database, accountId: string, freeLimit: number): Promise<DeviceAllowance> {
  const row = await db.prepare(
    "SELECT COUNT(*) AS n FROM device_upscales WHERE account_id = ? AND free = 1",
  ).bind(accountId).first<{ n: number }>();
  const freeUsed = row?.n ?? 0;
  return { freeLimit, freeUsed, freeRemaining: Math.max(0, freeLimit - freeUsed) };
}

type DeviceClaim = DeviceAllowance & { balance: number } & (
  | { kind: "free" | "paid" | "duplicate" }
  | { kind: "insufficient" }
);

/** Record one on-device upscale: free while the allowance lasts, then paid.
 *
 *  One D1 batch, which is a transaction. The free row writes only while the
 *  free count is under the limit; the spend writes only if no free row did and
 *  the balance covers it; the paid row writes only if that spend now exists.
 *  So "insufficient" leaves nothing behind, and a duplicate request id trips
 *  UNIQUE(account_id, request_id), rolling the whole batch back. */
export async function claimDeviceUpscale(
  db: D1Database,
  input: { accountId: string; requestId: string; freeLimit: number; credits: number },
): Promise<DeviceClaim> {
  const snapshot = async () => ({
    ...(await deviceAllowance(db, input.accountId, input.freeLimit)),
    balance: await creditBalance(db, input.accountId),
  });
  const existing = await db.prepare(
    "SELECT 1 FROM device_upscales WHERE account_id = ? AND request_id = ?",
  ).bind(input.accountId, input.requestId).first();
  if (existing) return { kind: "duplicate", ...(await snapshot()) };

  const id = crypto.randomUUID();
  const spendKey = `spend:device:${id}`;
  const now = Date.now();
  let results: D1Result[];
  try {
    results = await db.batch([
      db.prepare(
        `INSERT INTO device_upscales (id, account_id, request_id, free, credits, created_at)
         SELECT ?1, ?2, ?3, 1, 0, ?4
         WHERE (SELECT COUNT(*) FROM device_upscales WHERE account_id = ?2 AND free = 1) < ?5`,
      ).bind(id, input.accountId, input.requestId, now, input.freeLimit),
      db.prepare(
        `INSERT INTO credit_entries (account_id, delta, reason, idempotency_key, detail, created_at)
         SELECT ?1, ?2, 'spend', ?3, 'device:upscale', ?4
         WHERE NOT EXISTS (SELECT 1 FROM device_upscales WHERE id = ?5)
           AND (SELECT COALESCE(SUM(delta), 0) FROM credit_entries WHERE account_id = ?1) >= ?6`,
      ).bind(input.accountId, -input.credits, spendKey, now, id, input.credits),
      db.prepare(
        `INSERT INTO device_upscales (id, account_id, request_id, free, credits, created_at)
         SELECT ?1, ?2, ?3, 0, ?4, ?5
         WHERE EXISTS (SELECT 1 FROM credit_entries WHERE idempotency_key = ?6)`,
      ).bind(id, input.accountId, input.requestId, input.credits, now, spendKey),
    ]);
  } catch (error) {
    const raced = await db.prepare(
      "SELECT 1 FROM device_upscales WHERE account_id = ? AND request_id = ?",
    ).bind(input.accountId, input.requestId).first();
    if (raced) return { kind: "duplicate", ...(await snapshot()) };
    throw error;
  }
  const kind = (results[0]?.meta.changes ?? 0) > 0 ? "free"
    : (results[2]?.meta.changes ?? 0) > 0 ? "paid" : "insufficient";
